...
const Dashboard: React.FC<DashboardProps> = ({ user, onLogout, initialView = 'Dashboard' }) => {
  ...
  useEffect(() => {
    ...
    if (enableUnified && user && !staffRTC) {
      // Get API base URL - should point to server, not client
      const getApiBase = () => {
        ...
        if (typeof window !== 'undefined') {
          ...
          // If on staff dev server (5174) or client dev server (5173), use server port 8080
          if (port === '5174' || port === '5173') {
            return `http://${hostname}:8080`;
          }
          // If on unified server (8080), use current origin
          if (port === '8080' || window.location.pathname.startsWith('/staff')) {
      ...
      const ensureAvailability = async (tokenValue: string) => {
        try {
          let response = await fetch(`${apiBase}/v1/staff/availability`, {
            method: 'POST',
            headers: {
          ...
          if (response.status === 401) {
            ...
            if (refreshToken) {
              try {
                const refreshResponse = await fetch(`${apiBase}/api/auth/refresh-token`, {
                  method: 'POST',
                ...
                if (refreshResponse.ok) {
                  ...
                  if (data.token) {
                    ...
                    response = await fetch(`${apiBase}/v1/staff/availability`, {
                      method: 'POST',
                      headers: {
                        'Content-Type': 'application/json',
