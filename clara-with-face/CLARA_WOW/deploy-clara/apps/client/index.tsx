                ...
                return utterance;
                    ...
                    for (let i = 0; i < utterances.length; i++) {
                        try {
                            ...
                            console.error(`[TTS] Error queuing utterance ${i + 1}:`, err);
                        }
                    }
                    
                    // Verify that utterances were queued
                    if (!window.speechSynthesis.speaking && !window.speechSynthesis.pending) {
...
REMEMBER: Respond ONLY in ${languageName}. Do NOT add English. Do NOT provide additional responses. This is your complete answer.]`;
            ...
            if (message.serverContent?.turnComplete && !base64EncodedAudioString && !isTTSActive) {
                ...
                if (responseText) {
                    ...
                    console.warn('[TTS] Turn complete but no response text available for TTS. responseTextForTTS:', responseTextForTTS?.substring(0, 50));
                }
            }
        };
    };
